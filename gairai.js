...
function compile(ast, options = {}) {
  ...
  const ensureGuard = (ctx) => {
    ...
    if (!ctx.__gairaiGuard || typeof ctx.__gairaiGuard !== "object") {
      ctx.__gairaiGuard = {
 ...
 * Runner
 * --------------------------- */
const __gairaiProgramCache = new Map();
const __gairaiFnIds = new WeakMap();
let __gairaiFnSeq = 0;
const __gairaiObjIds = new WeakMap();
let __gairaiObjSeq = 0;

function getFnCacheId(fn) {
  if (!fn || typeof fn !== "function") return "nofn";
  if (__gairaiFnIds.has(fn)) return __gairaiFnIds.get(fn);
  __gairaiFnSeq += 1;
  const id = `fn${__gairaiFnSeq}`;
  __gairaiFnIds.set(fn, id);
  return id;
}

function getObjCacheId(obj) {
  if (!obj || typeof obj !== "object") return "noobj";
  if (__gairaiObjIds.has(obj)) return __gairaiObjIds.get(obj);
  __gairaiObjSeq += 1;
  const id = `obj${__gairaiObjSeq}`;
  __gairaiObjIds.set(obj, id);
  return id;
}
...
function makeProgramCacheKey(source, compileOnly) {
  ...
  return `${strictVars}|${strictArity}|${String(maxOutput)}|${String(maxOps)}|${String(maxRuntimeMs)}|${String(maxLoopIters)}|${String(maxCallDepth)}|${String(maxCalls)}|${loggerId}|${externalsId}|${source}`;
}
...
function createGairaiRunner(source, compileOptions = {}) {
...
function clearGairaiCache() {
  __gairaiProgramCache.clear();
}

function runGairai(source, initialVars = {}, compileOptions = {}) {
  ...
  if (cache) {
    ...
    const hit = __gairaiProgramCache.get(cacheKey);
    if (hit) {
      ast = hit.ast;
      program = hit.program;
    } else {
      ...
      __gairaiProgramCache.set(cacheKey, { ast, program });
    }
  } else {
    ast = parse(source);
    program = compile(ast, compileOnly);
...
const createGaigaiRunner = createGairaiRunner;
const clearGaigaiCache = clearGairaiCache;
const runGaigai = runGairai;
...
if (typeof module !== "undefined" && require.main === module) {
  const result = runGairai(sample, {}, { strictArity: true });
  console.log("---- final ----");
