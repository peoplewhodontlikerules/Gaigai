...
function inferTypeFromValue(v) {
  ...
  if (isRairaiFn(v) || typeof v === "function") return tFn([], T_ANY);
  return T_ANY;
}
...
function matchesType(v, type) {
  ...
  if (t.kind === "fn") return isRairaiFn(v) || typeof v === "function";
  return true;
}
...
function createRairaiRunner(source, compileOptions = {}) {
...
function clearRairaiCache() {
  PROGRAM_CACHE.clear();
}

function runRairai(source, initialVars = {}, compileOptions = {}) {
  ...
  if (useCache && PROGRAM_CACHE.has(key)) runner = PROGRAM_CACHE.get(key);
    ...
    runner = createRairaiRunner(source, opts);
